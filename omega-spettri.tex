...
\tikzmarknode{mark1}{A}\rar[hook]\dar[onslide=<4->,equal]\&
X\rar[hook]\dar[onslide=<4->,equal]\&
X\cup\cone{A}\rar[onslide=<2->,hook]\dar[onslide=<5->]{\iso}\&
{\visible<2->{(X\cup\cone{A})\cup\cone{X}}}\rar[hook,onslide=<3->]\dar[onslide=<6->]{\iso}\&
{\visible<3->{\big((X\cup\cone{A})\cup\cone{X}\big)\cup\cone(X\cup\cone{A})}}\dar[onslide=<7->]{\iso}\\
...
{\visible<9>{\Sigma Y}}
\end{diagram}
\begin{tikzpicture}[overlay,remember picture,every node/.style={blue,anchor=base,draw,circle,font=\footnotesize,inner sep=2pt}]
\node[left=.3cm of  mark1] {\textbf{1}};
\node[left=.3cm of  mark2] {\textbf{2}};
\end{tikzpicture}
\end{onlyenv}
\begin{onlyenv}<-9>
\begin{center}
...
\&\&\hmaps{A,K}\dar{\iso}\ar[onslide=<19->,blue,ddl,bend right]\&\hmaps{X,K}\lar\dar{\iso}\&\hmaps{X/A,K}\lar\&\hmaps{\Sigma A,K}\lar\\
